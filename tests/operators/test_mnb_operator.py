...
def test_rate_exists():
    ...
    operator = MnbExchangeRateOperator(task_id="test", date=date(2023,1,16).isoformat())
    result = operator.execute({})
    assert result == expected

def test_rate_doesnt_exist():
    operator = MnbExchangeRateOperator(task_id="test", date=date(2023,1,15).isoformat())
